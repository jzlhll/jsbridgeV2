...
https://github.com/lzyzsd/JsBridge 1.0.4的版本还是挺好用的。基本的逻辑图在本文最后：使用的是：**shouldOverrideUrlLoading + Iframe.src**更新的相互通知的机制。
为了兼容android4.x，必须采用1.0.4版本。

我基于该框架，也是为了完全兼容以前的前端H5代码，实现了jsbridge2.0。目前公司日活顶峰50w+。不再支持4.4，支持androiod5.0+，使用evaluateJavascript和重写，代码上也能简化。
